...
SP(config)#int f0/0
SP(config-if)#desc Porta LAN de VILA GUILHERME
SP(config-if)#ip add 172.14.0.1 255.255.0.0
SP(config-if)#no shut
SP(config-if)#exit

SP(config)#int s0/0
SP(config-if)#desc PORTA WAN que LIGA VILA GUILHERME a ITAQUERA
SP(config-if)#ip add 15.0.0.1 255.0.0.0
SP(config-if)#no shut
...
SP(config-if)#ip add 14.0.0.2 255.0.0.0
SP(config-if)#no shut
...
SP(config-router)#net 172.14.0.0
